      ...
      <tbody>
        {items.map((item) => (
          <tr key={item.id || item._id} className="border-t">
            ...
            <td className="p-3 text-right"><button onClick={() => onEdit(item)} className="mr-2 text-blue-600"><Edit size={17} /></button><button onClick={() => onDelete(item.id || item._id)} className="text-red-600"><Trash2 size={17} /></button></td>
          </tr>
        ))}
      </tbody>
    </table>
